...
export const Gallery = ({ items, onOpen, onDelete }) => {
  const ref = useRef(null);
  useEffect(() => {
    const ctx = gsap.context(() => {
      const cards = gsap.utils.toArray(".gal-card");
      ...
      {items.length === 0 ? (
        ...
        >
          ...
          <p className="mt-2 max-w-sm text-sm text-muted-foreground">
            Type a prompt at the top of the page and your first presenter video will appear here.
          </p>
        </div>
      ) : (
